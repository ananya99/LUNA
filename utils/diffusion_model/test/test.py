...
from metrics.evaluation_statistics import (compute_contact, compute_RSSD, align_point_clouds, compute_spearman_correlation)
from utils.data.dataholder import DataHolder
from utils.data.load import (cell_class_decoding, compute_distance, position_normalize, to_dataframe)
...
def get_test_save_path(root_dir, cfg):
    ...
    test_save_path = os.path.join(root_dir, f"model_{model_name}_epoch_{str(epoch_index)}")
...
def process_single_sample(self, batch, positions_pred, test_save_path, sample_index):
    ...
    positions_pred, positions_true, cell_ID = mask_positions(positions_pred, positions_true, cell_ID, node_mask)
    ...
    log_dict = perform_evaluation(self.cfg.test.epoch_index, metadata_true, metadata_pred_normalized, unique_cell_classes, save_dir)
    log_dict.update({
...
def perform_evaluation(epoch, metadata_true, metadata_pred_normalized, unique_cell_classes, dir_path):
    """
    Evaluates model predictions using various metrics and generates a log dictionary.
    """
    plot_scatter_visualization(metadata_true, metadata_pred_normalized, unique_cell_classes, dir_path)
    
    spr_v, spr_p, spr_avg, spr_median = compute_spearman_correlation(metadata_true, metadata_pred_normalized)
    sum_rssd, mean_rssd, absolute_rssd = compute_RSSD(metadata_true, metadata_pred_normalized)
