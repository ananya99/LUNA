...
for directory in input_directories:
    ...
    print(f"Loaded TIFF with shape {tiff.shape}, {df.shape[0]} boundaries.")

    cell_boundary_vertices = extract_cell_boundary_vertices(df)
    items = list(cell_boundary_vertices.items())

    batch_idx = 0
    batch_tar_template = os.path.join(output_dir, f"{sample_name}_cell_images_batch_{{batch_idx}}.tar")

    for batch in chunks(items, size=1000):
        batch_tar_path = batch_tar_template.format(batch_idx=batch_idx)
        print(f"Saving batch {batch_idx} to {batch_tar_path}")

        with tarfile.open(batch_tar_path, "w") as tar:
            with concurrent.futures.ProcessPoolExecutor() as executor:
                futures = [
                    executor.submit(process_single_cell, cell_id, coords, tiff)
                    for cell_id, coords in batch
                ]
                ...
                for future in concurrent.futures.as_completed(futures):
                    ...
                    if result is None:
                        continue
                    cell_id, buffer = result
                    tarinfo = tarfile.TarInfo(name=f"{cell_id}.npy")
    ...
    print(f"🎉 Completed saving all cells for {sample_name}")
