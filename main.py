...
from utils.diffusion_model.setup.setup import (setup_callbacks, setup_dataset,
                                               setup_model, setup_trainer)
...
def main(cfg: DictConfig):
    ...
    cfg.general.local_saved_path = hydra.core.hydra_config.HydraConfig.get().runtime.output_dir
...
def test_model(cfg: DictConfig, datamodule, dataset_infos):
    ...
    checkpoints_paths = [os.path.join(checkpoints_parent_dir, item) for item in checkpoints_name_list]

    dataloaders_test = datamodule.test_dataloader()

    for checkpoint_path in checkpoints_paths:
        test_single_checkpoint(cfg, datamodule, dataset_infos, checkpoint_path, dataloaders_test)
...
def get_checkpoints_list(cfg: DictConfig, checkpoints_parent_dir: pathlib.Path):
    ...
    if cfg.test.checkpoints_name_list == "all":
        ...
        checkpoints_name_list = list(set(checkpoints_name_list) - set("last_epoch.ckpt"))
    else:
        checkpoints_name_list = cfg.test.checkpoints_name_list
    return checkpoints_name_list

def test_single_checkpoint(cfg: DictConfig, datamodule, dataset_infos, checkpoint_path, dataloader_test):
